...
function removeSkill(btn) {
  const tagText = btn.parentElement.textContent.trim();
  const skills  = getSkills().filter(s => s !== tagText);
  saveSkills(skills);
  renderSkills();
}
...
async function saveResume(e) {
  ...
  try {
    const res  = await fetch('php/save_resume.php', { method: 'POST', body: data });
